...
class EEPoseTerm(Term):
    def __init__(
        ...
        self.cpp_term = opt_term_cpp.EEPoseTerm(
            model, ee_frame_id, delta_q, self.target_pose_inv_list, num_threads
        )
    ...
    def _calc_pose_err_cpp(
        self, q: np.ndarray, target_pose_inv: pin.SE3, with_jacobian: bool = False
    ):
        ret = self.cpp_term.calc_pose_err(q, target_pose_inv, with_jacobian)
        if with_jacobian:
            return ret[0], ret[1]
        else:
            return ret[0]

    def _calc_pose_err_list_cpp(self, traj_q: np.ndarray, with_jacobian: bool = False):
        ret = self.cpp_term.calc_pose_err_list(traj_q, with_jacobian)
        if with_jacobian:
            return ret[0], ret[1]
        else:
            return ret[0]
    ...
    def _approx(self, traj_q: np.ndarray):
        ...
        pose_err0_list, J_list = self._calc_pose_err_list_cpp(
            q0_list, with_jacobian=True
        )
    ...
    def _eval(self, traj_q: np.ndarray):
        q_list = traj_q[self.timestep_list]
        pose_err_list = self._calc_pose_err_list_cpp(q_list, with_jacobian=False)
